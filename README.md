...
For OpenCode Go, run `/opencode-go:setup` and paste either the full `https://opencode.ai/workspace/<id>/go` URL or the raw workspace ID, followed by the dashboard's `auth` cookie. The cookie input is masked, validated before saving, and stored in `~/.config/opencode/opencode-quota/opencode-go.json`. `/opencode-go:clear` removes only this dashboard credential; it does not remove Pi's model API key. `OPENCODE_GO_WORKSPACE_ID` and `OPENCODE_GO_AUTH_COOKIE` remain supported and take precedence over the managed file.
