...
For OpenCode Go, run `/opencode-go:setup`. The command shows the clickable `https://opencode.ai/auth` entry point without opening a browser, then explains the two required values: the full `https://opencode.ai/workspace/<id>/go` page URL (or its raw workspace ID) and only the dashboard's `auth` cookie value. It also shows the browser path `F12 > Application > Storage > Cookies > opencode.ai > auth > Value`. The cookie input is masked, validated before saving, and stored in `~/.config/opencode/opencode-quota/opencode-go.json`. `/opencode-go:clear` removes only this dashboard credential; it does not remove Pi's model API key. `OPENCODE_GO_WORKSPACE_ID` and `OPENCODE_GO_AUTH_COOKIE` remain supported and take precedence over the managed file.
